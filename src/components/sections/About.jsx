...
import {
  PROFILE, SUMMARY, EDUCATION, EXPERIENCE, PUBLICATION, CERTIFICATIONS,
} from '../../data/profile';
    ...
    <div className="space-y-6 sm:space-y-10 max-w-5xl mx-auto pb-4 sm:pb-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-base sm:text-lg md:text-xl leading-relaxed font-medium text-slate-400 mt-2 sm:mt-3">
          <span className="text-yellow-500 font-black">{PROFILE.subtitle.split('·')[0].trim()}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-5">
        <motion.div
          ...
          className="bg-slate-900/40 p-4 sm:p-6 md:p-8 rounded-xl sm:rounded-2xl md:rounded-[2rem] border border-yellow-500/20 shadow-2xl backdrop-blur-md"
        >
          <div className="flex items-center gap-3 sm:gap-4 mb-4">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-yellow-500/10 flex items-center justify-center rounded-xl sm:rounded-2xl border border-yellow-500/20 shrink-0">
          ...
          <p className="text-yellow-400 text-xs sm:text-sm font-bold mt-1">{EDUCATION.school}</p>
        ...
        >
          <div className="flex items-center gap-3 sm:gap-4 mb-4">
            ...
            <div className="min-w-0">
              ...
              <span className="inline-flex items-center gap-1 text-[9px] sm:text-[10px] font-black text-yellow-400 uppercase tracking-widest">
                <span className="w-1.5 h-1.5 rounded-full bg-yellow-500 shrink-0" /> {PUBLICATION.venue} · {PUBLICATION.year}
              </span>
            </div>
          </div>
      ...
      >
        ...
        <h3 className="text-white font-black text-lg sm:text-xl md:text-2xl mb-6 sm:mb-8 flex items-center gap-3">
          <div className="w-1.5 sm:w-2 h-6 sm:h-8 bg-yellow-500 rounded-full shadow-[0_0_20px_rgba(234,179,8,0.5)] shrink-0" />
          Experience
        </h3>
        <div className="space-y-8">
          {EXPERIENCE.map((exp) => (
            <div key={exp.company} className="relative pl-5 sm:pl-8 md:pl-10 border-l-2 border-slate-800">
              <div className="absolute -left-[9px] sm:-left-[11px] top-0 w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-yellow-500 ring-4 ring-slate-950 shadow-[0_0_16px_rgba(234,179,8,0.8)]" />
              <div className="flex flex-wrap items-baseline justify-between gap-2 mb-1.5">
                <h4 className="font-black text-white text-base sm:text-lg md:text-xl tracking-tight flex items-center gap-2 flex-wrap">
                  <Briefcase className="w-4 h-4 text-yellow-500 shrink-0" aria-hidden />
                  {exp.role}
                </h4>
                <span className="text-[10px] sm:text-sm text-slate-500 font-bold shrink-0">{exp.period}</span>
              </div>
              <p className="text-yellow-400 font-bold mb-4 uppercase tracking-widest text-xs sm:text-sm">{exp.company}</p>
              <ul className="grid grid-cols-1 gap-2 sm:gap-3">
                {exp.highlights.map((item) => (
                  <li key={item} className="flex gap-2.5 items-start bg-slate-800/30 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-white/5">
                    <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-500 shrink-0 mt-0.5" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3">
            {CERTIFICATIONS.map((cert) => (
              <div key={cert.name} className="bg-slate-900/40 border border-white/5 rounded-xl p-3 sm:p-4 flex items-start gap-2.5 sm:gap-3">
                <Award className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-500 shrink-0 mt-0.5" />
                <div className="min-w-0">
                  ...
                  <p className="text-yellow-400 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider mt-1">{cert.type}</p>
                </div>
              </div>
            ))}
          </div>
